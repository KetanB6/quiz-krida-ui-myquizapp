...
import { Sparkles, Zap, Globe, Cpu, Loader2, Trophy, RefreshCcw, Timer } from 'lucide-react';
import toast, { Toaster } from 'react-hot-toast';

// Constants
const SECONDS_PER_QUESTION = 60; // 1 Minute
        ...
        <PageContainer>
            ...
            {!quizData ? (
                <GlassCard>
                    ...
                    <FormGrid>
                        ...
                        <PrimaryButton onClick={handleGenerate} disabled={isLoading || !formData.topic}>
                            {isLoading ? <><Loader2 size={20} className="spinner" /> Synthesizing...</> : "Start Exam"}
                        </PrimaryButton>
                    </FormGrid>
                </GlassCard>
            ) : (
                <ResultContainer>
                    ...
                    <QuestionGrid>
                        {quizData.map((q, idx) => {
                            // Show ONLY current question if not submitted, show ALL if submitted
                            if (!isSubmitted && idx !== currentQuestionIdx) return null;

                            return (
                                <QuestionCard key={idx} style={{ animationDelay: `0.1s` }}>
                                    <div className="q-num">Module {idx + 1} of {quizData.length}</div>
                                    <h3>{q.question}</h3>
                                    <div className="options-list">
                                        {[q.opt1, q.opt2, q.opt3, q.opt4].map((opt, i) => {
                                            const isSelected = userAnswers[idx] === opt;
                                            const isCorrect = opt === q.correctOpt;
                                            let statusClass = isSubmitted ? (isCorrect ? "correct" : (isSelected ? "wrong" : "")) : (isSelected ? "selected" : "");

                                            return (
                                                <div key={i} className={`opt ${statusClass}`} onClick={() => handleSelectOption(idx, opt)}>
                    ...
                    {!isSubmitted && (
                        <StickyFooter>
                            <SubmitButton onClick={handleNextQuestion}>
                                {currentQuestionIdx === quizData.length - 1 ? "Finish & Submit" : "Next Question"}
                            </SubmitButton>
                        </StickyFooter>
                    )}
                </ResultContainer>
...
// --- New Styled Components for Timer ---
...
const SubmitButton = styled(PrimaryButton)` padding: 12px 28px; font-size: 0.95rem; max-width: 400px; width: 100%; border-radius: 100px; `;

const ResultContainer = styled.div` width: 100%; max-width: 800px; z-index: 1; padding-bottom: 100px; `;

const ResultHeader = styled.div`
    display: flex; flex-direction: column; margin-bottom: 32px; gap: 10px;
    .success-badge, .score-badge { padding: 6px 14px; border-radius: 100px; font-size: 0.8rem; font-weight: 700; display: inline-flex; align-items: center; gap: 8px; align-self: flex-start; }
    ...
    h2 { margin: 8px 0; font-size: 1.8rem; }
    .reset-btn { background: rgba(255,255,255,0.05); border: 1px solid rgba(255,255,255,0.1); color: #fff; padding: 12px 24px; border-radius: 12px; cursor: pointer; display: flex; align-items: center; gap: 8px; align-self: flex-start; }
`;

const QuestionGrid = styled.div` display: flex; flex-direction: column; gap: 24px; `;
